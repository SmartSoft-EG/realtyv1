...
d-page(@refresh="str.fetchRealty()")
    template(#tools)
        v-btn(color="primary" @click="addRealty")
            | Add realty
    q-table(title="Realty" :filter="search" flat :rows="str.realty_list" :columns="columns" row-key="id")
        template(#top-right)
        ...
        template(#body-cell-options="props")
            td.flex.gap-2
                v-btn(icon="mdi-eye" variant="outlined" density="compact" color="success"  :to="'/realty/' + props.row.id")
    ...
    d-dialog(v-model="str.show_add_dialog" title="Add realty" @save="saveRealty()")
